...
public class LeaveApprovalsPanel extends JPanel {
    ...
    private JPanel createStatCard(String title, String value, Color color) {
        ...
        switch (title) {
            case "PENDING": pendingValueLabel = valueLabel; break;
            case "APPROVED": approvedValueLabel = valueLabel; break;
            case "REJECTED": rejectedValueLabel = valueLabel; break;
    ...
    private JPanel createTablePanel() {
        ...
        leaveTable.getColumnModel().getColumn(6).setCellRenderer(new DefaultTableCellRenderer() {
            ...
            public Component getTableCellRendererComponent(JTable table, Object value,
                ...
                if ("PENDING".equals(status)) setForeground(UITheme.ACCENT_ORANGE);
                else if ("APPROVED".equals(status)) setForeground(UITheme.ACCENT_GREEN);
                else if ("REJECTED".equals(status)) setForeground(UITheme.ACCENT_RED);
    ...
    private void filterData() {
        ...
        if ("ALL".equals(filter)) {
            ...
            filtered = allRequests.stream().filter(r -> r.getStatus() == status).toList();
        }
    ...
    private void searchRequests() { filterData(); }
    ...
    private void viewDetails() {
        ...
        details.append("Employee: ").append(request.getEmployeeName()).append("\n");
        ...
        JOptionPane.showMessageDialog(this, scrollPane, "LEAVE REQUEST DETAILS", JOptionPane.PLAIN_MESSAGE);
    }

    private void approveRequest() {
        LeaveRequest request = getSelectedRequest();
        if (request == null || request.getStatus() != LeaveRequest.LeaveStatus.PENDING) return;

        String remarks = controller.showInput("Approval remarks:", "");
        if (remarks != null && controller.showConfirm("Approve request?", "CONFIRM")) {
            try {
                if (employeeService.approveLeaveRequest(request.getRequestId(), currentUser.getFullName(), remarks)) {
                    controller.showInfo("Approved");
                    loadData();
                }
    ...
    private void rejectRequest() {
        ...
        if (reason != null && !reason.trim().isEmpty() && controller.showConfirm("Reject request?", "CONFIRM")) {
            try {
                if (employeeService.rejectLeaveRequest(request.getRequestId(), currentUser.getFullName(), reason)) {
                    controller.showInfo("Rejected");
                    loadData();
                }
            } catch (Exception e) { controller.showError(e.getMessage()); }
        }
