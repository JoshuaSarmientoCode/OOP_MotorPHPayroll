...
public abstract class BaseDAO<T> implements DataService, DAOInterface<T> {
    ...
    // ========== CSV UTILITY METHODS (available to all DAOs) ==========
    ...
    protected double parseCurrency(String value) {
        if (value == null || value.trim().isEmpty() || value.equals("N/A")) return 0.0;
        try {
            String clean = value.replace("\"", "").replace(",", "")
                    .replace("₱", "").replace("PHP", "").trim();
