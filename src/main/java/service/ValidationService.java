...
public class ValidationService {
    ...
    public ValidationResult validateGovernmentIds(GovernmentIds gov) {
        ...
        // SSS (optional for display)
        if (!isNullOrEmpty(gov.getSssNumber()) && 
            !SSS_PATTERN.matcher(gov.getSssNumber()).matches()) {
            result.addFieldError("sss", "Invalid SSS format (XX-XXXXXXX-X)");
        }
        
        // TIN (optional for display)
        if (!isNullOrEmpty(gov.getTinNumber()) && 
            !TIN_PATTERN.matcher(gov.getTinNumber()).matches()) {
            result.addFieldError("tin", "Invalid TIN format (XXX-XXX-XXX-XXX)");
        }
        
        // PhilHealth (optional for display)
        if (!isNullOrEmpty(gov.getPhilHealthNumber()) && 
            !PHILHEALTH_PATTERN.matcher(gov.getPhilHealthNumber()).matches()) {
            result.addFieldError("philHealth", "Invalid PhilHealth format (XX-XXXXXXXXX-X)");
        }
        
        // Pag-IBIG (optional for display)
        if (!isNullOrEmpty(gov.getPagIbigNumber()) && 
            !PAGIBIG_PATTERN.matcher(gov.getPagIbigNumber()).matches()) {
            result.addFieldError("pagIbig", "Invalid Pag-IBIG format (XXXX-XXXX-XXXX)");
        }
